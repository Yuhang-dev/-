<%@page import="main.java.myblog.po.ResortsBean"%>
...
<div class="col-md-7">
	...
	<table class="offset-1 table table-hover table-sm" style="text-align:justify;">
		<tbody>
<%
	List<ResortsBean> totalList=((RecordsList)session.getAttribute("recordlistbean")).getResortsRecordsList();
	int currentPage=0;
	if(request.getParameter("resortsmanagePage")==null){
		currentPage=0;
	}

	else{currentPage=Integer.parseInt(request.getParameter("resortsmanagePage"));}
	int size=totalList.size();
	List<ResortsBean> list=null;
	int pagesize=3;
	...
	for(int i=0;i<list.size();i++){
		ResortsBean bean=list.get(i);
		out.print("<tr><td>");
		out.print(bean.getRName());
		out.print("<td>");
		out.print(bean.getRInfo());
		out.print("</td><td>");
		out.print("<a href=\"data?actionName=changeResorts"+bean.getRName()+"\""
		+"class=\"text-warning\" method=\"post\">");
		out.print("修改</a></td>");
		out.print("<td><a href=\"data?actionName=deleteResorts&name="+bean.getRName()
		+"\""+"class=\"text-danger\" onclick=\"return confirmAction();\" method=\"post\">");
		out.print("删除</a></td></tr>");
	}
	
	out.print("</tbody></table>");
	out.print("</tbody></table><a href=\"user?actionName=resortsdatamanage&resortsmanagePage="+(currentPage-1)

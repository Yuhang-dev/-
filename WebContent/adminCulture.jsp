<%@page import="main.java.myblog.po.CultureAdminBean"%>
...
<%
	List<CultureAdminBean> list=(List<CultureAdminBean>)session.getAttribute("AdminCultureList");
	int currentPage=0;
	if(request.getParameter("cultureadminPage")==null){
		currentPage=1;
	}

	else{currentPage=Integer.parseInt(request.getParameter("cultureadminPage"));}
	...
	for(int i=0;i<list.size();i++){
		CultureAdminBean bean=list.get(i);
		out.print("<tr><td>");
		...
		out.print("</td><td>");
		out.print(bean.getCPic());
		out.print("</td><td>");
		out.print("<a href=\"AdminData?actionName=changeCulture&name="
		+bean.getCName()+"\""+"class=\"text-warning\" method=\"post\">");
		out.print("修改</a></td>");
		out.print("<td><a href=\"AdminData?actionName=deleteCulture&name="+bean.getCName()+"&id="+bean.getUserId()+"\""
		+"class=\"text-danger\" onclick=\"return confirmAction();\" method=\"post\">");
		out.print("删除</a></td></tr>");
	}
	
	out.print("</tbody></table>");
	out.print("</tbody></table><a href=\"AdminServlet?actionName=Culture&page="+(currentPage-1)
